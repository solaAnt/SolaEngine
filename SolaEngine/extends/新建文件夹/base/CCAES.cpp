...
#define PUTU32(ciphertext, st) { (ciphertext)[0] = (uint8_t)((st) >> 24); \
	...
	*/
	static int32_t rijndaelSetupEncrypt(uint32_t* rk, const uint8_t* key, int32_t keybits)
	{
		int32_t i = 0;
		uint32_t temp;
	...
	* @return the number of rounds for the given cipher key size.
	*/
	static int32_t rijndaelSetupDecrypt(uint32_t* rk, const uint8_t* key, int32_t keybits)
	{
		int32_t nrounds, i, j;
		uint32_t temp;
	...
	static void rijndaelEncrypt(const uint32_t *rk, int32_t nrounds, const uint8_t plaintext[16], uint8_t ciphertext[16])
	{
	...
	static void rijndaelDecrypt(const uint32_t *rk, int32_t nrounds, const uint8_t ciphertext[16], uint8_t plaintext[16])
	{
		uint32_t s0, s1, s2, s3, t0, t1, t2, t3;
	...
	void AES::EncryptData(uint8_t *contents, uint32_t num_bytes, const aes_key &key)
	{
		int32_t nrounds;
		uint32_t rk[RKLENGTH(AES_KEYBITS)] = { 0 };

		assert((num_bytes & (AESBlockSize - 1)) == 0);

		// Set up the rk buffer
		nrounds = rijndaelSetupEncrypt(rk, (const uint8_t*)&key[0], AES_KEYBITS);

		// Encrypt the data a block at a time
		for (uint32_t offset = 0; offset < num_bytes; offset += AESBlockSize)
		{
			rijndaelEncrypt(rk, nrounds, contents + offset, contents + offset);
		}
	}

	void AES::DecryptData(uint8_t *contents, uint32_t num_bytes, const aes_key &key)
	{
		int32_t nrounds;
		uint32_t rk[RKLENGTH(AES_KEYBITS)] = { 0 };
		...
		// Set up the rk buffer
		nrounds = rijndaelSetupDecrypt(rk, (const uint8_t*)&key[0], AES_KEYBITS);
